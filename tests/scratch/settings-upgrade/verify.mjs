...
check('默认面板 = 外观（首个 system section）', text.includes('切换到亮色') || text.includes('切换到暗色'))
...
check('仅列 articles 源（Gaubee 文章）', text.includes('Gaubee 文章') && !text.includes('Gaubee 说说'))
...
text = await page.evaluate(() => document.body.innerText)
check('说说源面板（Gaubee 说说，无文章源）', text.includes('Gaubee 说说') && !text.includes('Gaubee 文章'))
...
check('底栏出现备案号 + GitHub 链接', footerText.includes('闽ICP备17026139号-1') && footerText.includes('GitHub'))
const beianHref = await footer.getByRole('link', { name: '闽ICP备17026139号-1' }).getAttribute('href')
...
await page.locator('a[href^="/article/articles/"]').first().waitFor({ state: 'visible', timeout: 15000 })
